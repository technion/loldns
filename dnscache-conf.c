...
int main(int argc,char **argv)
{
  ...
  snprintf(loguid_s, 64, "%d\n", pw->pw_uid);
  snprintf(gid_s, 64, "%d\n", pw->pw_gid);
  seed_addtime(); makedir("log");
  seed_addtime(); owner(pw->pw_uid,pw->pw_gid);
  seed_addtime(); perm(0744);
  seed_addtime(); start("log/fatals"); outs("Fatal errors:\n"); finish();
  seed_addtime(); owner(pw->pw_uid,pw->pw_gid);
  seed_addtime(); perm(0664);
  ...
  seed_addtime(); perm(0644);
  seed_addtime(); start("env/GID"); outs(gid_s); finish();
  seed_addtime(); perm(0644);
  seed_addtime(); start("env/UID"); outs(uid_s); finish();
  seed_addtime(); perm(0644);
  seed_addtime(); start("env/LOGUID"); outs(loguid_s); finish();
  seed_addtime(); perm(0644);
  ...
  seed_addtime(); perm(0644);
  snprintf(fdmax, 8, "%d\n", MAXUDP+50);
  seed_addtime(); start("env/FDLIMIT"); outs(fdmax); finish();
  seed_addtime(); perm(0644);
  ...
  /* I failed at making any sense of DJB's API to do this his way */
  dirs = fopen("/etc/sysconfig/dnscache", "a");
  if(!dirs)
  {
