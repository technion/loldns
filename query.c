...
struct ip_mask bogon_list[MAXBOGONS];

int okclient_ip_net_cmp (unsigned char ip[4], unsigned char base[4], unsigned char netmask[4])
{
    if ((unsigned)(ip[0]&netmask[0]) == base[0] &&
            (unsigned)(ip[1]&netmask[1]) == base[1] &&
            (unsigned)(ip[2]&netmask[2]) == base[2] &&
            (unsigned)(ip[3]&netmask[3]) == base[3])
        return 1;
...
static int doit(struct query *z,int state)
{
  ...
  while (i < k) {
	...
	char tmpip[4];
	   ...
	   byte_copy(tmpip,4,header);
	   for(r = 0; bogon_list[r].netmask[0] != 0; r++)
	   {
	  
	     if(okclient_ip_net_cmp(tmpip, bogon_list[r].base, bogon_list[r].netmask))
		goto NXDOMAIN; 
	  }
          save_data(header,4);
